        ...
        <div v-for="(realEstate, index) in realEstate" :key="index" class="min-req-box ">
          <div class="min-req-title-imge-delete">
            <div class="min-req-title-imge">
              ...
              <div class="info-header">
                <h2 class="info-head min-req-info-head">{{ getproperty_typeById(realEstate.property_type) }}</h2>
              </div>
            </div>
            <div class="delete-sec" > <!-- @click="deletePartner(partner.id)" -->
              <div class="min-req-imge-delete">
          ...
          <section class="actions">
            ...
            <router-link to="/fackFullRequest" class="details-link">
              التفاصيل
            </router-link>

            <div class="another-acrions">
              <button  class="mashrouk-btn">تحديث</button> <!-- @click="updatePartner(partner.id)" -->
              <button  class="mashrouk-btn m-r-20" style="background-color: red;">ايقاف</button> <!-- @click="stopPartner(partner.id)" -->
            </div>
          </section>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get(
          `http://abdo6755-001-site1.anytempurl.com/Home/GetmyPosts?Iduser=${this.Iduser.replace(/"/g, '')}`,
          { headers },
        );

        this.realEstate = response.data.real_Estate_No;
        ...
        this.realEstateyes = response.data.real_Estate_Yes;
        console.log('Number of elements in real_yes:', this.realEstateyes.length);
      } catch (error) {
    ...
    shortenId(id) {
    // You can use a library like hashids or implement a custom hash function
    // For simplicity, let's use a basic hash function (not recommended for security purposes)
